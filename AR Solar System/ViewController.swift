...
class ViewController: UIViewController {
    ...
    func simulation() {
        ...
        // PLANETS
        let mercury = planet(geometry: SCNSphere(radius: 0.05), diffuse: UIImage(named: "mercuryDiffuse")!, specular: nil, emission: nil, normal: nil, position: SCNVector3(0.5, 0, 0))
        ...
        let saturn = planet(geometry: SCNSphere(radius: 0.2), diffuse: UIImage(named: "saturnDiffuse")!, specular: nil, emission: nil, normal: nil, position: SCNVector3(2, 0, 0))
        ...
        let neptun = planet(geometry: SCNSphere(radius: 0.15), diffuse: UIImage(named: "neptuneDiffuse")!, specular: nil, emission: nil, normal: nil, position: SCNVector3(2,0,0))
        ...
        let mercuryParentRotation = rotation(time: 5)
        mercuryParent.runAction(mercuryParentRotation)
        let venusParentRotation = rotation(time: 6)
        venusParent.runAction(venusParentRotation)
        let earthParentRotation = rotation(time: 7)
        earthParent.runAction(earthParentRotation)
        let marsParentRotation = rotation(time: 8)
        marsParent.runAction(marsParentRotation)
        let jupiterParentRotation = rotation(time: 9)
        jupiterParent.runAction(jupiterParentRotation)
        let saturnParentRotation = rotation(time: 10)
        saturnParent.runAction(saturnParentRotation)
        let uranusParentRotation = rotation(time: 10)
        uranusParent.runAction(uranusParentRotation)
        let neptunParentRotation = rotation(time: 11)
